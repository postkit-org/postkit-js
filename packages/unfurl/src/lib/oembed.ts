...
export function normalizeOEmbed(
  value: unknown,
  options: NormalizeOEmbedOptions,
): ResolvedLinkPreview {
  ...
  const mediaUrl =
    type === 'photo' ? safeHttpUrl(stringValue(data, 'url')) : undefined;
  const thumbnailUrl = safeHttpUrl(stringValue(data, 'thumbnail_url'));
  const image: ResolvedLinkImage | undefined = mediaUrl
    ? {
        src: mediaUrl,
  ...
  return {
    ...
    url:
      safeHttpUrl(stringValue(data, 'url')) ??
      options.fallbackUrl ??
    ...
    embed:
      type !== 'link' || rawHtml
        ? {
            ...
            height: embedHeight,
            aspectRatio:
              embedWidth && embedHeight ? embedWidth / embedHeight : undefined,
            title: stringValue(data, 'title'),
            rawHtml,
          }
        : undefined,
